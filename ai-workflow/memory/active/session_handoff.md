...
- Status: draft
- Updated: 2026-07-03 (rev 47→48: PR #14 self-review follow-up 보완 5건 amend. `StatusPill` canonical 12-state union 중 `BUILD_SUCCESS`/`TEST_SUCCESS`/`DEPLOY_SUCCESS`/`COMPLETED` 를 `--color-accent-success` 로 통일 (success 의미 색상), `BuildDetail.svelte` `.kv` grid `@media (max-width: 640px)` 1컬럼 fallback 추가 (narrow viewport horizontal scroll 차단), canonical lifecycle section h2 `Lifecycle` → `Build lifecycle` 로 차별화 (meta row 짧은 라벨과 분리), Legacy preview 안내문을 canonical section 참조 + removal plan 명시 형태로 명확화, `BuildRow.svelte` `BuildRowData` 에 `lifecycleStatus` optional prop 추가 + `StatusPill` 에 전달. 회귀: TS 4 packages clean, Go 9 packages, build-server 52/52, build-monitor svelte-check 0/0. workflow meta 동기화 (state rev 68→69, handoff 47→48, work_backlog 40→41, backlog §49 Result 봉인 + §50) 같은 amend commit 에 포함. main HEAD `5e597c3` (CodeWhale overlay sync) 에 rebase.)
