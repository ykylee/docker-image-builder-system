...
- Updated: 2026-08-07 (rev 176→177: v0.12.0 follow-up — Revoke / Lease 영속화 + 자동 갱신 — 운영 가이드 + **v0.12.0** release staging 정합). 신규 schema `revoke_jti` (migration 0019) + `PersistentRevokeStore` + `HmacIdentityProvider.persistentRevokeStore` 옵션 + `LeaseRenewer` background worker (30s sweep + 만료 30% 시점 expiresAt += TTL + 이전 jti revoke). 운영 가이드 [`docs/operations/revoke-lease-persistence-2026-08-06.md`](../../docs/operations/revoke-lease-persistence-2026-08-06.md) 신규 8 섹션. PROJECT_PROFILE §3.15 신규. CHANGELOG §2 v0.12.0 entry 신규 + §1 release history 의 v0.12.0/v0.11.0/v0.10.0 row + 현재 release 갱신 + §28 회귀 baseline 표 v0.11.0/v0.12.0 컬럼 추가 (17 컬럼). state.json `current_baseline` `v0.11.0`→`v0.12.0` 갱신 (G6 drift 해소) + `purpose_digest_rev 264→265` / `handoff_rev 176→177` / `index_rev 142→143` / `latest_rev 13→14`. 회귀: build-server **295/295 PASS** (이전 285 + v0.12.0 follow-up 10 신규: lease-renewer 4 + persistent-revoke-store 6) / TS 5 packages `--noEmit` clean / `git diff --check` clean. 운영 자산 무변경: build-monitor / runner / SQL 변경 0. code 변경 0 (단, 신규 5 file + 4 file amend — schema/migration/build-server/src/auth/lease-renewer.ts/build-server/src/auth/persistent-revoke-store.ts/build-server/src/auth/hmac-identity-provider.ts/build-server/src/services/build-service.ts/build-server/src/app/create-app.ts). follow-up: git tag `v0.12.0` annotated tag 발매 (사용자 결정 대기, TASK-131 정합 정책) + 운영 환경 staging 검증 (운영 가이드 §3 의 `psql \d revoke_jti` + `/auth/runner-login` + `/auth/logout` 6 단계 + LeaseRenewer 자동 갱신 실측) + v0.12.0 follow-up 5종 (revoke_jti cleanup sweeper / LeaseRenewer multi-replica 정합 / fail-open 정책의 운영 metric + alert / admin 강제 전체 로그아웃 endpoint / DISABLED 토글 시 즉시 lease 회수).
