...
- Updated: 2026-08-03 (rev 169→170: **v0.8.16 post-merge session close — Helm/ArgoCD adapter 구현 + PR #60 리뷰 수정 및 squash merge 완료**). `origin/main`은 `39b7025`를 가리키며 Helm hosted slice, ArgoCD Application adapter, Build Request hosting options, Admin Hosted Services 스타일 보강이 반영됐다. 검증: runner `go test ./...`/`go vet ./...`, frontend `tsc --noEmit`, Vitest 26 files/282 tests, PR CI `build + smoke` PASS. 다음 세션: ArgoCD 실 클러스터 e2e, Helm/ArgoCD hosted HTTP e2e, v0.9.0 release 정리. 주의: PR merge로 `docs/PROJECT_PROFILE.md`가 45줄로 축소된 상태라 문서 무결성/복구 검토가 필요하다.
