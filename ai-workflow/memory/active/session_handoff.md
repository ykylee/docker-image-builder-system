...
- Updated: 2026-08-04 (rev 170→171: **v0.9.0 release candidate e2e 완료 + PROJECT_PROFILE 복구**). Helm kind e2e, hosted HTTP kind e2e, ArgoCD Application kind e2e가 모두 PASS했다. `docs/PROJECT_PROFILE.md`는 원본 이력 기준 352줄로 복구하고 v0.9.0/ArgoCD 현황을 반영했다. 5개 package version은 `0.9.0`으로 준비했고 `CHANGELOG.md`와 `docs/RELEASE_NOTES-v0.9.0-2026-08-04.md`를 갱신했다. 남은 작업은 workflow state 정합 검토와 사용자 승인 후 commit/tag/push 결정이다. 자동 session-end 스크립트는 `workflow_kit.common.maturity` 누락으로 실행되지 않아 수동 가드를 적용한다.
