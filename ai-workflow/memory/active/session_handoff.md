...
- Updated: 2026-08-06 (rev 175→176: Phase 2 — Runner 인증 (HMAC lease token) 1·2차 봉인 — 운영 가이드 + **v0.11.0** release staging 정합). 신규 라우트 `/auth/runner-login` + `/auth/runner-lease-renew` + ClaimResponse.leaseToken + ClaimResponse.expiresAt + HmacIdentityProvider.issueWithExpiresAt + BuildService.getRunnerStatus + enforceRunnerLease helper + Runner 측 LoginLease/RenewLease/EnsureLease (atomic.Value lease 보관). 운영 가이드 [`docs/operations/runner-lease-hmac-2026-08-06.md`](../../docs/operations/runner-lease-hmac-2026-08-06.md) 신규 8 섹션. PROJECT_PROFILE §3.14 신규. CHANGELOG §2 v0.11.0 entry 신규 (Phase 2 1·2차 봉인). compose.dev.yaml env `RUNNER_LEASE_TTL_SECONDS` (default 300) 추가. 5 package.json 0.10.0 통일 (변경 0). 회귀: build-server **285/285 PASS** (이전 278 + Phase 2 신규 7 case: login 4 + renew 3) / runner Go **8 packages PASS** (stub interface EnsureLease/RenewLease/LoginLease no-op 추가) / TS 5 packages `--noEmit` clean / `git diff --check` clean. 운영 자산 무변경: build-monitor / SQL / schema / migration 변경 0. code 변경 0 (단, Phase 1 1·2차 봉인과 동일 wire format / secret 동일). follow-up: git tag `v0.11.0` annotated tag 발매 (사용자 결정 대기, TASK-131 정합 정책) + 운영 환경 staging 검증 (release-checklist §3 Verify 의 Phase 2 6 단계) + v0.12.0 Phase 2 follow-up 후보 5종 (메모리 revoke Set 멀티 replica 영속화 / long-running build lease 자동 갱신 worker / secret rotation 자동화 / Phase 2 3·4·5차 봉인 — Runner별 K8s RBAC / build sandbox 격리 / resource limits + node/namespace 분리 / DISABLED 토글 시 즉시 lease 회수 WebSocket/SSE 채널). 운영 가이드 [`docs/operations/identity-cookie-hmac-2026-08-06.md`](../../docs/operations/identity-cookie-hmac-2026-08-06.md) 신규 8 섹션 (의도 / 결정 / 4단계 회귀 baseline / 운영 명령 Production·Self-dogfood·단위 / 사용 절차 cookie·Bearer·legacy / 운영 환경 배포 절차 4단계 / 사전 결함 + 보강 4건 / 운영 환경 baseline / 한계와 follow-up 5종 / 결정). PROJECT_PROFILE §3.13 Phase 1 운영 패턴 신규 (결정 / 신규 회귀 가드 32 case / 핵심 변경 5 파일 / 운영 명령 / 운영 검증 절차 6 단계 / 사전 결함 + 보강 4건 / 운영 영향 / 회귀 baseline / follow-up 5종). CHANGELOG.md §2 v0.10.0 entry 신규 (1·2단계 / 3단계 / 4단계 / 운영 환경 baseline / 운영 명령 / 업그레이드 안내 / follow-up). §1 release history 의 v0.10.0 row + 현재 release 갱신. §28 회귀 baseline 표 v0.10.0 컬럼 추가 (16 컬럼). §30 다음 release 가이드 v0.10.0→v0.11.0. 5 package.json `0.9.0`→`0.10.0` 통일 bump (G5 가드 정합). release-checklist §3 Verify 보강 — Phase 1 Identity + 테넌트 권한 회귀 가드 (whoami 401 hint / login v2 cookie round-trip / admin builds 200 / admin 비통과 403 callerId echo / logout jti revoke + cookie 만료) 6 단계. compose.dev.yaml env 4종 추가 — `AUTH_HMAC_SECRET` (default 빈 문자열, 운영 필수), `AUTH_TOKEN_TTL_SECONDS` (default 28800), `AUTH_LEGACY_HEADERS` (default false), `BUILD_OWNER_POLICY_LEGACY_DEFAULT_SUBJECT` (default `<anonymous>`). state.json `current_baseline` `v0.9.0`→`v0.10.0` 갱신 (G6 drift 해소). 회귀: build-server **278/278 PASS** (변경 0 — 본 TASK 는 docs only + 운영 메타 + version bump 만). TS 5 packages `--noEmit` clean / `git diff --check` clean / session-end 가드 9/9 PASS (release commit 후). 운영 자산 무변경: build-monitor / runner / SQL / schema / migration 변경 0. code 변경 0. follow-up: git tag `v0.10.0` annotated tag 발매 (사용자 결정 대기, TASK-131 정합 정책) + 운영 환경 staging 검증 (release-checklist §3 Verify 의 Phase 1 6 단계) + v0.11.0 follow-up 후보 5종 (메모리 revoke Set 영속화 Redis/Postgres + SSO/OIDC/JWT + route-level preHandler 일원화 + legacy X-User-Id/X-Admin-Id sunset + 멀티 탭 logout broadcast channel).
- Updated: 2026-08-06 (rev 173→174: Phase 1 — Identity + 테넌트 권한 4단계 봉인: admin-routes requireAdmin preHandler 단일화). helper `enforceAdminGuard(request, reply, isAdmin, { legacyHeadersEnabled })` 가 모든 admin-scoped 라우트 (14개: `/admin/builds`, `/admin/hosted-services/*`, `/admin/hosting-capacity`, `/admin/users`, `/admin/admins/*`, `/admin/runners/*`) 의 inline 가드를 일원화. cookie/Bearer 인증 admin + legacy ON X-Admin-Id 헤더 allow-list 일치 → 통과, legacy ON + X-Admin-Id 비-allow-list → 403 + callerId echo, legacy OFF + X-Admin-Id → 401 + Authentication required + hint. legacyHeadersEnabled **default OFF** (3단계 build-routes 와 정합). `requireAdmin` preHandler 등록 라인은 dead code 로 정리 (handler 내부 helper 호출이 동일 envelope 재현) — 향후 route-level preHandler 활성화는 fastify route config 일원화 TASK 에서 별도 봉인. helper 가 callerId 를 반환하지 않으므로 `resolveAdminCallerId` helper 를 추가해 `updateServiceManifest` 의 owner 인자에 전달. 신규 회귀 가드 `apps/build-server/tests/admin-routes-prehandler.test.ts` 6 case + 기존 `admin-routes.test.ts` / `admin-runners-routes.test.ts` `buildAppWithService` 가 `AUTH_LEGACY_HEADERS=true` default 로 legacy ON 명시. 회귀: build-server **278/278 PASS** (이전 272 + 신규 6). TS 5 packages `--noEmit` clean / `git diff --check` clean / session-end 가드 9/9 PASS. 운영 자산 무변경: build-monitor / runner / SQL / schema / migration / 5 package.json (`0.9.0`) 변경 0. 다음: 5단계 — 운영 가이드 `docs/operations/identity-cookie-hmac-2026-08-06.md` + PROJECT_PROFILE §3.13 + CHANGELOG v0.10.0 + compose/k8s manifest env (`AUTH_HMAC_SECRET`, `BUILD_OWNER_POLICY_LEGACY_DEFAULT_SUBJECT`) + release-checklist §3 Verify 에 whoami curl + v2 wire format 마이그레이션 안내.
- Updated: 2026-08-06 (Phase 1 — Identity + 테넌트 권한 1차 봉인: HMAC 서명 Bearer cookie + principal preHandler). production-readiness-roadmap Phase 1 의 1차 봉인. `X-User-Id`/`X-Admin-Id` 평문 헤더 위조 + localStorage 인증 결함을 해결하는 신규 표면을 도입. `apps/build-server/src/auth/` (identity-provider.ts + hmac-identity-provider.ts + request-principal.ts) + `apps/build-server/src/routes/auth-routes.ts` (`/auth/login` + `/auth/logout` + `/auth/whoami`) 신규. `packages/shared-contract/src/auth/` 신규 (Principal/AuthLoginRequest/AuthLoginResponse/WhoAmIResponse schema). `create-app.ts` 가 HmacIdentityProvider 를 boot 시 instantiate 하고 `registerPrincipalPreHandler` 로 `request.principal` 채움. openapi.ts 의 `getOpenApiDocument()` 에 cookieAuth + bearerAuth securityScheme 추가. CORS allowHeaders 에 Cookie 추가. 결정: AUTH_LEGACY_HEADERS **default false** (마이그레이션 경로) — 기존 self-dogfood 의 X-Admin-Id 호출은 그대로 통과, 신규 cookie 인증은 운영자가 명시적으로 켤 때 동작. 회귀: build-server **257/257 PASS** (기존 243 + 신규 14). build-monitor / runner / SQL / schema / migration / 5 package.json 변경 0. 다음: build-routes / admin-routes 의 owner policy 강제 (3단계, 본 세션 보류), 운영 가이드 + CHANGELOG v0.10.0 (5단계).
...
- Updated: 2026-08-05 (현재 구현 실사용성 셀프 리뷰). Build Server 243/243, React 289/289, Go `go test ./...`, Skill/MCP 227건, TypeScript check와 메인 `/health`·`/api/services`·`/docs` smoke를 통과했다. 결론은 **신뢰된 내부 self-dogfood/MVP에는 사용 가능, 외부 다중 사용자 실서비스에는 미준비**다. CRITICAL/HIGH: `X-Admin-Id`/`X-User-Id`·localStorage는 인증이 아니며 타 사용자/관리자 신원 위조가 가능하고, Runner claim/phase/deploy API도 무인증이다. 비신뢰 Dockerfile에 host Docker socket과 kubeconfig를 공유한다. 또한 kind control-plane이 별도 memory backend 단일 replica라 main Postgres와 데이터가 분리되고, claim lease/reaper 부재로 Runner 장애 시 stuck build가 가능하며, 성공 build workspace/image cleanup이 없다. DB Secret provisioning 실패 후 `PROVISIONING` 조기 반환으로 자동 재시도가 안 되는 lifecycle 결함도 확인했다. 다음 우선순위는 인증/테넌트 권한 → Runner·빌드 격리 → control-plane Postgres/HA 및 claim recovery → DB provisioning 보상·재시도 → cleanup/observability다.

- Updated: 2026-08-05 (메인 `/api/services` 404 수정). `GET /services` 서버 route는 존재했지만 production `/api/*` rewrite 허용 목록에 `/services`가 빠진 것이 원인이었다. `/services` rewrite와 OpenAPI path/schema를 추가하고 Build Server static-serve/OpenAPI 테스트 24/24, OpenAPI TypeScript 재생성을 통과했다. 메인 이미지를 재빌드·재기동한 뒤 `X-User-Id: admin`으로 `/api/services` → `/services` 307 및 최종 서비스 목록 JSON을 실측했다.

- Updated: 2026-08-05 (메인 서비스 API Docs 동일 반영). 공통 `ApiConsole`의 context-aware docs URL과 viewport 기반 세로 레이아웃 수정이 self-hosted 서비스뿐 아니라 메인 `dibs-build-server`에도 실제 반영되도록 이미지를 재빌드·재기동했다. 메인 `BUILD_REPOSITORY_BACKEND=postgres`와 Postgres `DATABASE_URL`은 유지됐다. `http://100.119.181.116:3000/api-console`, `/docs/`, `/health`가 `200`, `200`, `{"status":"ok"}`이며, Playwright 브라우저에서 Scalar endpoint list와 확장된 iframe 영역을 확인하고 console error 0을 기록했다. 캡처 `output/playwright/api-console-main.png`.

- Updated: 2026-08-05 (API Docs 세로 레이아웃 개선). 1280x720 실브라우저에서 Scalar iframe이 기존 `min-height:480px`로 좁은 것을 확인해 API Console page/frame을 viewport 기반으로 확장(`min-height:640px`, `height:72dvh`, mobile 620px)했다. ApiConsole Vitest 289/289, React typecheck, Vite build PASS. build `9eb67357-874a-4272-a0bb-f8520c79d3c0` `COMPLETED`; 최종 URL에서 endpoint list 표시 및 console error 0을 확인했다.

- Updated: 2026-08-05 (API Docs context 경로 및 self-host DB 검증). `ApiConsole`의 root-absolute docs/OpenAPI 링크를 `/<context>/docs/`, `/<context>/openapi.json`으로 수정했고 실제 Scalar iframe endpoint list 및 browser console error 0을 확인했다. 기존 서비스가 manifest/Secret/DB backend 미설정(memory)이었던 것을 발견해 `database.enabled=true` manifest와 schema/role/Secret을 provision했다. Runner K8s manifest가 DB opt-in 시 `DATABASE_URL` Secret과 `BUILD_REPOSITORY_BACKEND=postgres`를 함께 주입하도록 보완했고 이미지 기본 backend는 pre-deploy container-test를 위해 memory로 유지했다. build `abf070a8-6030-49b7-a15b-11022c019146` `COMPLETED`; pod 로그 migration `0001~0018`, gateway 경유 DB probe(`current_user`, 전용 schema, database `dibs`, probe=1), Secret metadata, Pod restart 후 persistence를 확인했다. 최종 URL은 `http://100.119.181.116:18080/docker-image-builder-system-admin-20260805-api-fixed/`. 참고: `/context/api/*`는 cluster-local control-plane API이고 `/context/*`는 self-host 앱 API다.

- Updated: 2026-08-05 (Admin self-hosted deep-link/API 경로 수정). 실제 브라우저 오류의 1차 원인은 `/context/admin/assets/*`가 `index.html`로 응답되는 자산 경로였고, 2차 원인은 `openapi-fetch`의 `./api` 문자열 결합으로 `/context/admin/api/admin/*`를 호출하는 API base였다. Build Server의 deep asset/favicon fallback과 React `getApiBaseUrl() -> /<context>/api`를 수정했다. Build Server 242/242, frontend Vitest 288/288, typecheck/diff check PASS. 수정본 build `6a0e56dd-fbd1-42cb-beb7-dfe2c0411ca1`가 `COMPLETED`; 최종 URL `http://100.119.181.116:18080/docker-image-builder-system-admin-20260805-api-fixed/`. admin 로그인 후 Builds와 5개 Admin API(빌드/사용자/관리자/러너/서비스) 200, browser console error 0을 확인했다.

- Updated: 2026-08-05 (서비스 DB migration gate). kubectl/Helm/ArgoCD Secret 주입을 정렬하고 앱 Pod `initContainer` 기반 migration gate를 구현했다. Helm render, Runner `go test ./...`, Build Server `236/236`을 통과했다. manifest별 command 전달과 full kind migration e2e가 다음 작업이다.

- Updated: 2026-08-05 (사용자 Services 탭 및 런타임 URL 정합). Builds와 분리된 `/services` 사용자 탭과 `GET /services`(`X-User-Id`)를 추가해 사용자의 HostedService만 조회하도록 memory/Postgres 저장소를 연결했다. `AppHeader` Services 링크, 운영 URL/상태/현재 build/replica 카드 UI를 추가했다. 컨테이너 테스트 전용 `container-test.local`은 public `runtimeUrl`에서 제거하고 기존 Postgres row도 null 정리했다. K8s hosted deployment의 `http://<HOSTING_BASE_HOST>/<contextPath>/` 보고 규칙은 유지한다. 검증: build-server 233/233, frontend 284/284, typecheck/image build PASS. 실제 `100.119.181.116:3000/services`는 200, `/admin/builds` 문서 200, admin JSON의 내부 hostname 미검출.
- Updated: 2026-08-05 (관리자 전체 서비스 조회 경로). AdminTabs에 `/admin/services`를 canonical Services 탭으로 추가하고 기존 `/admin/hosting`은 하위 호환 alias로 유지했다. `/admin/hosted-services` API는 관리자 전체 HostedService 목록을 반환한다. 실제 IP에서 `/admin/services` HTML 200, 비인증 API 401, admin API 200(`services: []`)을 확인했다. 현재 목록이 빈 것은 영구 HostedService가 아직 생성되지 않았기 때문이다.
- Updated: 2026-08-05 (admin self-dogfood 재검증). `requestedBy=admin`으로 frontend dist를 `dockerfileMode=auto` 제출했고 build `35823857-d331-4ceb-ad03-2801f43ab30e`가 source upload부터 `COMPLETED`까지 성공했다. admin builds filter에서 확인했으며 `runtimeUrl=null`, `/admin/hosted-services`는 현재 hosting base 미설정으로 `services: []`다. 추가로 public build logs에서 내부 `container-test.local` URL을 제거하고 기존 Postgres 로그 2건을 정리했다. targeted build-server 44/44, typecheck/image build PASS.
- Updated: 2026-08-05 (self-dogfood 정상 실행 모드 및 필터 진단). 기존 Compose Runner가 기본 `skeleton` build/run이라 실제 처리가 아니었던 것을 확인하고 Compose 기본을 Docker `cli` build/run으로 변경했다. 첫 실제 실행 `6417b465-1764-439e-82f0-4687cacfd7de`는 33.9초 후 Runner 내부 loopback probe 결함으로 실패했고, `RUNNER_DOCKER_HOST=host.docker.internal` 보정 후 `af07773b-ff84-42b1-940c-bb9ee164f865`가 4.4초에 실제 Docker build/run + container test + COMPLETED를 통과했다. `COMPLETED` filter의 lifecycleStatus 누락도 수정했다. 현재 Compose는 K8s hosting이 아니므로 `/admin/services`가 비는 것은 별도 hosting adapter가 필요한 상태다. React 285 tests, Go runner targeted tests, image build PASS.

- Updated: 2026-08-05 (서비스 기동 및 프로젝트 self-dogfood 실측). 기존 Docker Compose의 `dibs-build-server`/Postgres/Runner가 healthy/up 상태임을 확인했고, 격리 compose에서 `e2e-dockerfile-auto-lifecycle.sh`를 실행했다. static·Node source는 generated Dockerfile → BuildKit build/run → terminal `COMPLETED`, unsupported source는 `FAILED` + `DOCKER_BUILD_FAILED`로 모두 PASS. e2e compose cleanup 후 기존 서비스 생존도 확인. 당시 `ai-workflow/wiki/index.md` 누락을 확인했으며 2026-08-06 wiki anchor 구축으로 해소했다.
- Updated: 2026-08-05 (프로젝트 frontend 산출물 기반 Dockerfile 생성 self-dogfood). `apps/build-monitor/dist-react`를 source archive로 제출했다. 기존 stale Runner image는 `Dockerfile not found`로 실패했으나 최신 Runner image rebuild/restart 후 `generated Dockerfile ... template=static-nginx` 로그를 확인했고, 실제 build/test/deploy가 `COMPLETED`했다. 성공 build `1f4edebf-7c08-4048-9e75-bd8968559059`. 현재 build-server/Postgres/Runner는 healthy/up이다.
- Updated: 2026-08-05 (self-dogfood preview 공개). 완료 후 자동 정지되는 test container 대신 동일 `dist-react`를 `dib-selfdogfood-preview` nginx container로 띄워 `http://127.0.0.1:38124/` HTML `200`을 확인했다. Build Server API는 `http://127.0.0.1:3000`에서 제공된다.
- Updated: 2026-08-05 (IP 접속 404 보정). `compose.dev.yaml`의 React dist 경로 불일치로 `100.119.181.116:3000/`이 404였고, `BUILD_MONITOR_REACT_DIST_PATH=/app/apps/build-monitor/dist`를 추가해 build-server image를 rebuild했다. 이후 IP 기준 `/`, `/health`, frontend asset, `/builds` 모두 `200`; 로그에 `react dist mounted (single SPA)` 확인.
- Updated: 2026-08-05 (현재 서비스 self-dogfood 재실행). `http://100.119.181.116:3000`에 frontend archive를 제출해 Build ID `781ac9b6-dbb0-4c41-8841-cce2086d5273`를 생성했다. Runner의 `template=static-nginx` 자동 생성과 build/test/deploy 전체 `COMPLETED`를 확인했다.
- Updated: 2026-08-05 (SPA/API 경로 404/JSON 노출 보정). `Sec-Fetch-Dest`가 없는 HTML 문서 요청도 `Accept: text/html` 기준으로 SPA fallback을 타도록 `create-app.ts`를 수정하고 회귀 테스트를 추가했다. static-serve 20/20 + typecheck clean 후 build-server image를 rebuild했다. 실제 `100.119.181.116:3000/builds`와 `/admin/builds`가 HTML 200, API 요청은 JSON 유지.
- Updated: 2026-08-05 (history persistence/API client 보정). Compose 기본 backend를 Postgres로 고정해 memory 재기동 시 이력 소실을 막고, production React API client는 `/api` redirect 없이 same-origin route를 직접 호출한다. build-monitor 284/284 PASS. 현재 서비스에 Postgres self-dogfood build `3a5edbc1-519e-43fd-b590-fb62670cbe0e`를 재실행해 `COMPLETED`, admin builds 영속 이력 3건 확인.
...
- Updated: 2026-08-04 (TASK-176: Runner BuildKit 전환 완료). Runner runtime에 Alpine `docker-cli-buildx`를 추가하고 real build 호출에 `DOCKER_BUILDKIT=1`을 명시했다. 이미지 내부 `docker buildx v0.14.0` 확인 및 auto Dockerfile 전체 lifecycle e2e ALL PASS(static/Node COMPLETED, unsupported DOCKER_BUILD_FAILED, legacy builder 경고 없음). 다음: BuildKit cache/export 또는 CI buildx 정책은 별도 후보.
- Updated: 2026-08-05 (로컬 hosting 환경 셋업 완료). 기존 `kind-dib`에 ingress-nginx를 설치하고 `100.119.181.116:18080` 포트포워딩을 유지한다. Compose build-server/runner에 named kubeconfig volume과 K8s hosting 환경을 연결했으며, runner Docker socket gid `991` 권한도 반영했다. admin self-dogfood `47185c31-f54c-49d9-90a3-272c721f5f1a`가 실제 Docker build/run, container test, K8s Deployment/Service/Ingress를 거쳐 `COMPLETED`; runtime HTTP 200, deployment 1/1, `/admin/hosted-services` 및 `/services`에서 RUNNING 서비스 1건을 확인했다. 다음 세션은 포트포워딩 프로세스 영속화와 asset/sub-path 회귀 확인이 후속이다.
- Updated: 2026-08-05 (Runtime URL 링크 대비 수정). `BuildDetail.css`의 runtime URL 링크가 전역 bare-anchor accent 색상에 덮여 파란 배경에서 보이지 않던 specificity 충돌을 `.runtime-url-box a.runtime-url-link`로 봉인했다. frontend production build와 build-server image를 재생성했고 `/health` 200, Vitest 12건, React typecheck, diff check를 통과했다.
- Updated: 2026-08-05 (path-hosted self-dogfood 빈 페이지 수정). Vite `base: "./"`로 Ingress context path 내부의 상대 asset URL을 생성하고, React `BrowserRouter`에 unknown first path segment를 basename으로 자동 적용했다. 동일 URL `http://100.119.181.116:18080/self-dogfood-k8s-admin-retry/`로 재배포한 build `4edbeaf6-2d71-4466-909b-4d315de05428`가 `COMPLETED`; Playwright에서 `/self-dogfood-k8s-admin-retry/login` 렌더링, console error 0을 확인했다. 이전 서비스는 sandbox quota 때문에 새 배포 전 admin delete로 정리했다.
- Updated: 2026-08-05 (path-hosted API 404 수정). 로그인 후 `/builds` API가 static nginx runtime으로 향하던 문제를 path-hosted production API base URL이 현재 host의 `:3000` Build Server를 가리키도록 수정했다. frontend 전체 Vitest `286/286` 및 typecheck 통과. 동일 context path 최종 build `3f58cebc-9461-4cbd-9ed7-f1268f697c7d`를 배포하고 admin 로그인 후 `/self-dogfood-k8s-admin-retry/builds` 목록/API 호출, browser console error 0을 확인했다.
- Updated: 2026-08-04 (TASK-174: Dockerfile auto 전체 lifecycle e2e 봉인). API `dockerfileMode=auto` 전달부터 source upload/claim/Runner 자동 생성/실제 Docker build-run/terminal까지 static·Node 성공과 unsupported 실패를 실측했다. static nginx 템플릿을 Runner 기본 포트 8080으로 정렬하고, e2e compose의 고정 이름·포트·Colima socket GID·tar 경로 함정을 보강했다. 검증: lifecycle ALL PASS, Runner `go test ./...`, shell syntax, `git diff --check` PASS. 다음: 전체 lifecycle은 봉인되었고 BuildKit 전환은 별도 운영 후보다.
- Updated: 2026-08-04 (rev 171→172: v0.9.0 release 봉인 및 tag 발급). Helm kind e2e, hosted HTTP kind e2e, ArgoCD Application kind e2e가 모두 PASS했고, `docs/PROJECT_PROFILE.md`를 원본 이력 기준 352줄로 복구했다. 5개 package version `0.9.0`, CHANGELOG/release notes, workflow meta를 정합시킨 뒤 v0.9.0 release commit과 annotated tag를 발급한다. 다음: ArgoCD e2e nightly CI 및 Helm/ArgoCD hosted HTTP 통합 e2e 검토. 자동 session-end 스크립트는 `workflow_kit.common.maturity` 누락으로 실행되지 않아 수동 가드를 적용한다.
- Updated: 2026-08-04 (rev 172→173: **통합 deployment e2e nightly CI 편입**). `.github/workflows/nightly-e2e.yml`의 `deployment-e2e` job이 Helm install/status/uninstall, hosted HTTP path/subdomain/lifecycle, ArgoCD Application sync/health/cleanup을 순차 실행하도록 확장됐다. Helm setup과 Linux kind Docker network gateway 기반 ArgoCD local Git source 접근을 추가했다. 다음: 실제 scheduled/manual workflow run 결과 관찰 후 v0.10.0 후보(webhook 확장, sync cache 실측) 우선순위 검토. 자동 session-end 스크립트의 기존 `workflow_kit.common.maturity` 모듈 누락은 별도 follow-up이다.
- Updated: 2026-08-03 (Helm adapter vertical slice — release cleanup 식별자 정합 + hosted chart Ingress + kind e2e ALL PASS). `K8sCleanupOptions.HelmRelease`를 추가했고 Helm values에 `hosting.scheme`/`hosting.baseHost`를 전달한다. 검증: runner `go test ./...`, `helm lint`, `helm template`, `bash apps/runner/scripts/e2e-helm-deploy.sh` 모두 PASS. 다음: ArgoCD adapter 또는 Ingress controller 결합 hosted HTTP e2e.
- Updated: 2026-08-03 (ArgoCD adapter 구현 — `RUNNER_K8S_MODE=argocd`, Git source Application CR 생성, Helm parameter 전달, Synced/Healthy 대기, foreground cleanup, worker wiring). `cd apps/runner && go test ./...` PASS. 실 ArgoCD cluster e2e와 Helm/ArgoCD hosted HTTP e2e는 후속 검증.
- Updated: 2026-07-27 (rev 168→169: **v0.8.16 release 봉인 — session-end 가드 9종 확장 (G6~G9 신설) + CHANGELOG §26 표 v0.8.15 row 추가 (patch)**). 표준 ai-workflow 키트 session-end skill 의 가드 5종을 **9종으로 확장**. G6: `current_baseline` ↔ CHANGELOG.md latest release 정합 / G7: HEAD commit subject 정합 (release commit 의 vX.Y.Z ↔ handoff_rev 의 G3 잔존 검출) / G8: session_handoff.md 첫 줄 `- Updated:` 헤더 ↔ CHANGELOG.md latest release 정합 / G9: state.json semantic 검증 (필수 필드 / 타입 / `latest_backlog_path` 2중복 단일화 정합). 적용 후 2/9 fail 검출 (G2 current_baseline drift + G8 session_handoff 첫 줄 ↔ CHANGELOG drift — 본 release commit 시점에 모두 해소). CHANGELOG.md §26 회귀 baseline 표 v0.8.15 row + v0.8.16 row 추가 (총 14 컬럼, 운영 가드 행에 `session-end 9종 가드` 추가, workflow meta 정합 행에 `session-end 가드 9종` 추가). 5 package.json `0.8.15`→`0.8.16` 통일. workflow meta 갱신: state.json `purpose_digest_rev 208→209` / `handoff_rev 168→169` / `index_rev 130→131` / `latest_rev 4→5`. workflow / 스크립트 / SQL / schema / migration 변경 0. 회귀 baseline: TS 5 clean / build-server 205 / build-monitor vitest 278(사전 환경 의존 1 fail v0.8.3 동일) / migration 0001~0012 / go 8 pkg / **session-end 가드 9/9 PASS** (G2/G8 drift는 release commit 시점 current_baseline first 줄 + CHANGELOG §26 표 갱신으로 해소). follow-up: 가드 9종 unit test 추가 (G6~G9 회귀 가드) / G9 의 2중복 정합 apply 모드 검토 / state.json schema 단일화 follow-up (TASK-115/116 historical 본문 source_of_truth.latest_backlog_path 언급 정합) / v0.8.17 patch / v0.9.0 minor 진입 결정 (Helm/ArgoCD adapter).
...
## 2026-08-04 서비스 manifest 관리 1차 vertical slice 완료

- `ServiceManifest` shared contract와 revision 응답 스키마를 추가했다.
- PostgreSQL migration `0016_service_manifest.sql`, memory/Postgres current manifest 및 immutable revision 저장을 구현했다.
- admin API 3종(`GET/PUT manifest`, `GET revisions`)과 OpenAPI/CORS를 갱신했다.
- Build Monitor `/admin/hosting`에 manifest JSON 조회·수정·revision 저장 UI를 연결했다.
- 검증: build-server 232 PASS, frontend Vitest 284 PASS, 관련 TypeScript clean, `git diff --check` PASS.
- `e75709f feat: add service manifest management`를 push해 `origin/main`에 반영했다.
- 현재 범위는 admin-only CRUD/revision이다. Helm/ArgoCD 실제 apply·rollback, 사용자 owner 권한/화면, 배포 성공 시 자동 seed는 다음 작업이다.

## 2026-08-05 격리 self-dogfood 및 Dockerfile 조건 보완

- host `:3000` API fallback을 제거하고 path-hosted bundle의 API base를 `./api`로 고정했다. Ingress `/context/api/*`가 cluster-local `dib-control-plane:3000` Service로 전달된다.
- auto Dockerfile은 static SPA fallback/8080, Node `PORT=8080`을 기본 보장하며 root-absolute asset을 사전 거부한다.
- `examples/k8s-control-plane.yaml`을 추가해 kind self-dogfood control-plane을 재현 가능하게 했다.
- `self-dogfood-k8s-admin-isolated`에서 `/api/health`, `/api/builds`, HTML/assets가 200이고 control-plane 로그에 Ingress 내부 요청이 확인됐다.
- runner `go test ./...` 및 frontend Vitest `286/286`, React typecheck PASS. control-plane memory backend는 host Postgres 이력과 분리된 현재 의도이며, 운영 공유 DB는 후속 과제다.

## 2026-08-05 서비스별 DB 격리 설계 착수

- DB는 호스트 PostgreSQL 하나로 유지하되 platform DB와 service DB를 분리하고, 서비스별 schema/role/Secret을 발급하는 설계를 작성했다.
- 서비스 pod는 host IP에 직접 접근하지 않고 cluster-local `dib-db-gateway`를 통해서만 Postgres protocol을 사용한다.
- `docs/design/service-database-isolation.md`와 `.omx/plans/service-database-isolation.md`에 provisioning, migration, NetworkPolicy, purge/rotation, 수용 기준을 기록했다.
- 1차 vertical slice로 `ServiceManifest.database` strict opt-in 계약, `packages/db`의 `service_database` schema, migration `0017_service_database.sql`, bootstrap DDL을 추가했다. host/schema/role/password caller input은 거부한다.
- `service-database-provisioner.ts`에 stable schema/role/Secret name 생성과 advisory-lock transaction provisioning helper를 추가했다. 기존 metadata가 있으면 password를 재반환하지 않는다.
- admin DB provisioning API와 `KubectlSecretWriter`를 연결했다. Secret 값은 kubectl argv/log가 아닌 stdin YAML의 base64 data로만 전달한다. gateway host 미설정 시 503이며 schema/role 생성도 하지 않는다.
- kubectl Deployment manifest에 optional `DATABASE_URL` Secret reference와 hosted-service label을 추가했고, `examples/k8s-db-gateway.yaml`에 HAProxy gateway와 서비스→gateway NetworkPolicy를 추가했다. upstream 확정 전 live apply는 보류했다.
- kind live 검증: gateway Running, host PostgreSQL 왕복 select 1, `db-demo` provisioning 201/READY, Secret data 존재, role current_schema 확인, public schema write 거부, runtime 200. 현재 quota는 control-plane/app/gateway 3 pods 기준이다.
- Helm/ArgoCD adapter에도 `database.secretName` parameter와 hosted-service label/optional `DATABASE_URL` template을 반영했고 Helm template render 및 runner Go 전체 테스트를 통과했다.
- 실검증에서 migration 0017의 id default 누락을 발견해 0018 보정 migration을 추가하고 실제 bootstrap 적용을 확인했다. 다음은 Helm/Argo 경로, migration Job, full service e2e다.
- 아직 구현 전이며 gateway 구현체, TLS/firewall, migration 실행 정책은 후속 결정 사항이다.
- kubectl/Helm/ArgoCD에 서비스 DB Secret 주입을 정렬했고, migration은 별도 Job 대신 앱 Pod `initContainer` rollout gate로 구현했다. Helm render, Runner `go test ./...`, Build Server `236/236` 테스트를 통과했다.
...
- 전역 migration env 의존성을 제거했다. Build Server claim 응답에는 manifest의 `database.enabled`/`migrationCommand`만 포함되고, Runner가 서비스별 Secret reference와 migration `initContainer`를 구성한다. credential/host/schema는 전달하지 않는다.
- full kind migration e2e에서 initContainer ResourceQuota 누락을 발견해 kubectl/Helm 양쪽에 앱과 동일한 resource profile을 추가했다. 실제 migration exit 0, 앱 컨테이너 시작, 서비스 schema table 생성, public schema 쓰기 거부를 확인했고 임시 Deployment/quota는 정리했다.
- `GET /admin/hosted-services/:appName/database` 상태 조회 API/OpenAPI를 추가하고 Compose에서 `db-demo` READY 상태를 확인했다. 응답에는 schema/role/Secret 이름과 migration 정책만 있고 credential은 없다. Build Server 238/238 PASS.
- Admin 서비스 목록에 DB 상태 column을 연결했다. `READY`/migration revision을 표시하고 미프로비저닝 서비스는 `Not provisioned`로 안전하게 표시한다. 프론트 287/287, React typecheck, 최신 Build Server image rebuild PASS.
- DB lifecycle purge API를 추가했다. 기본 hosted service 삭제는 DB를 보존하며, exact appName confirmation을 받은 purge만 Secret 삭제 후 schema/role/metadata를 제거한다. live 잘못된 confirmation `400` 및 기존 `db-demo READY` 보존을 확인했고 Build Server 239/239 PASS.
- DB Secret rotation API를 추가했다. exact appName confirmation으로 기존 schema/role/data를 유지한 채 role password와 Kubernetes Secret을 교체한다. live `db-demo` rotation 후 새 Secret으로 gateway 접속해 `current_user`, `current_schema`, `migration_gate_probe`를 확인했고 Build Server `240/240` PASS. TLS/firewall 운영 경계는 후속이다.
- gateway egress NetworkPolicy를 host DB upstream CIDR(`192.168.5.2/32`)와 DNS로 제한해 kind에 적용했고 `db-demo` 접속을 재확인했다. 사용자 결정에 따라 외부 HTTPS와 PostgreSQL TLS는 도입하지 않는다.
- 환경별 DB upstream CIDR 정책을 기본 gateway 매니페스트에서 `examples/k8s-db-gateway-egress.yaml`로 분리했다. kind 정책은 `192.168.5.2/32`로 재적용됐고 운영 적용 시 클러스터별 주소로 교체한다.
- 실제 hosted app Pod에서 gateway `5432` 허용, host PostgreSQL 직접 접근 및 control-plane `:3000` 차단을 확인했다. namespace quota가 3 pods로 가득 차 기존 app Pod를 probe로 사용했다.
- Dockerfile 생성과 skill/MCP 정책을 정렬했다. Runner Dockerfile generator는 `DATABASE_URL`/`DB_*`/`PG*` ENV·ARG 선언을 거부하고, `build-request-intake`도 `SERVICE_DATABASE_POLICY_VIOLATION`으로 동일하게 거부한다. 자동 생성/추천 문서와 root Dockerfile에 platform Secret 주입 규칙을 추가했다. Go dockerfile/deploy, Python 16/16, Build Server 240/240 PASS.
- admin self-dogfood를 `requestedBy=admin`으로 완료했다. `dockerfileMode=auto` static sample이 skill 검증, source upload, `static-nginx` 자동 생성, Docker build, container test, K8s deploy까지 성공했고 runtime 및 `/api/health` 200, admin Builds/Services 반영을 확인했다. 1차 quota 실패를 계기로 sandbox control-plane quota를 2 CPU/2Gi/3 pods로 보완했다. retry service는 admin DELETE로 제거했고 기존 샘플은 replicas=1로 복구했다.
- 직전 URL 404 대응으로 같은 context path를 build `8011da1e-62a0-4b47-8f4e-925eaa4995ed`로 재배포했다. runtime와 `/api/health` 200, Admin Services RUNNING을 확인했고 서비스를 유지 중이다. 기존 샘플은 quota 때문에 replicas=0이다.
- 프로젝트 자체를 admin으로 self-hosting했다. 최종 build `9cfea142-2f7a-4e7a-a163-ee1af5370aa1`가 `COMPLETED`; `/`, `/health`, `/openapi.json` 200 및 Admin Services RUNNING을 확인했다. 과정에서 archive `.generated` 필수 파일, `/health` runner probe, stale Ingress, claim runtimePort→container test 전달 결함을 보완했고 root Dockerfile에 React dist env를 추가했다. 현재 URL은 `http://100.119.181.116:18080/docker-image-builder-system-admin-20260805/`; DB manifest는 opt-in하지 않아 memory backend로 동작한다.
