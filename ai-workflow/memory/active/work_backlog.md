...
- 상태: stable
- 최종 수정일: 2026-07-03 (rev 35→36: CodeWhale overlay 배포 — standard_ai_workflow v0.11.22 의 CodeWhale 하네스를 본 저장소에 적용, `.codewhale/skills/codewhale-workflow/SKILL.md` 단일 파일 emit. state.json commands 주입, 기존 root 진입점 보존.) PR #12 squash merge `1af82d0` main 합류. 브랜치 정리. 다음: PR #13.
...
- [x] TASK-052: CodeWhale overlay 배포 — standard_ai_workflow v0.11.22 (cf0060d, v0.10.4) 가 추가한 CodeWhale 하네스를 본 저장소에 적용. `render_codewhale_skill()` (bootstrap_lib.harnesses.renderers) 직접 호출로 `.codewhale/skills/codewhale-workflow/SKILL.md` 단일 파일 emit. Constitution additive rule 만 포함 (Constitution Article II/III/Regulations/Statutes 와 중복 주입 금지). 기본 검증 명령 (lint/type_check/quick_test) 은 state.json 의 real commands 주입 (tsc --noEmit 4 packages + apps/runner go build). full bootstrap (`bootstrap_workflow_kit.py --harness codewhale`) 은 memory state docs (state.json / session_handoff.md / work_backlog.md / daily_backlog / assessment) 를 덮어쓰려 하므로 우회 — 기존 AGENTS.md / MiniMax.md / 메모리 8종 보존. 메모리 sync: state rev 61→62, handoff 34→35, work_backlog 35→36. 회귀 영향 없음 (신규 1 파일, 기존 entry point 무변경). 후속: 첫 CodeWhale 세션에서 SKILL.md 가 `.codewhale/skills/` 로 인식되는지 + 검증 명령이 동작하는지 확인.
