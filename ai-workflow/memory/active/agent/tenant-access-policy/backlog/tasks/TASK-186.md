...
- 진행 현황: 잘못된 access token audience 거부와 custom OIDC_ADMIN_ROLE 외 admin role 403 회귀를 추가했다.
- 다음 세션 시작 포인트: Keycloak 연결 가능 환경에서 실제 audience mapper와 realm role smoke를 확인한 뒤 TASK-186 종료 검토.
- 남은 리스크: 실제 Keycloak access token aud/role mapper는 네트워크 복구 전 미검증.

## ✅ Outcome

- 작업 결과: fake issuer negative audience 및 dib-admin guard negative case를 테스트로 고정했다.
- 검증 결과: principal.test.ts 20/20 PASS; git diff --check PASS.
- 후속 작업:
