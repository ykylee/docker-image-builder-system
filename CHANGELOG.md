...
- 상태: stable (v0.5.1 갱신 — HTTPS 미도입 확정)
- 최종 수정일: 2026-07-24
...
- **MAJOR.MINOR.PATCH** — `v0.5.1` 형식
- **현재 release**: `v0.5.1` (2026-07-24, HTTPS 미도입 확정)
- **release history**:
...
- **5 package.json 통일 정책**: `apps/build-server` / `apps/build-monitor` / `packages/shared-contract` / `packages/shared-config` / `packages/db` 의 version field 가 모두 동일하게 유지되어야 함. `apps/runner` 는 Go module 이라 version field 없이 git tag 로 버전 관리. 본 release 시점 5/5 `0.5.1` 로 통일.
...
## 2. v0.5.1 (2026-07-24) — HTTPS 미도입 확정 (patch)
...
## 3. v0.5.0 (2026-07-24) — subdomain 호스팅 스킴 (minor)
...
## 4. v0.4.1 (2026-07-24) — 호스팅 e2e nightly CI 편입 (patch)
...
## 5. v0.4.0 (2026-07-24) — Phase 3 완료 (호스팅 능력)
...
## 6. v0.3.0 (2026-07-24) — Phase 2 완료 (preview-era 청산 → 배포 능력)
...
## 7. v0.2.1 (2026-07-23) — Phase 1 후속 패치
...
## 8. v0.2.0 (2026-07-22) — Phase 1 완료 baseline
...
## 9. v0.1.0 (2026-07-20) — 백엔드/운영 성숙도 baseline
...
## 10. 회귀 baseline 종합 (TASK-088 → v0.5.0)
...
- `v0.5.2` — patch (회귀 baseline 변경 0 + 운영 가이드/CI 소폭)
