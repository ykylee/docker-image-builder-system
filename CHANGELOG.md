...
- **MAJOR.MINOR.PATCH** — `v0.6.0` 형식
- **현재 release**: `v0.6.0` (2026-07-24, 실패 경로 e2e)
- **release history**:
...
- **5 package.json 통일 정책**: `apps/build-server` / `apps/build-monitor` / `packages/shared-contract` / `packages/shared-config` / `packages/db` 의 version field 가 모두 동일하게 유지되어야 함. `apps/runner` 는 Go module 이라 version field 없이 git tag 로 버전 관리. 본 release 시점 5/5 `0.6.0` 로 통일.
...
## 2. v0.6.0 (2026-07-24) — 실패 경로 e2e (minor)
...
## 3. v0.5.1 (2026-07-24) — HTTPS 미도입 확정 (patch)
...
## 4. v0.5.0 (2026-07-24) — subdomain 호스팅 스킴 (minor)
...
## 5. v0.4.1 (2026-07-24) — 호스팅 e2e nightly CI 편입 (patch)
...
## 6. v0.4.0 (2026-07-24) — Phase 3 완료 (호스팅 능력)
...
## 7. v0.3.0 (2026-07-24) — Phase 2 완료 (preview-era 청산 → 배포 능력)
...
## 8. v0.2.1 (2026-07-23) — Phase 1 후속 패치
...
## 9. v0.2.0 (2026-07-22) — Phase 1 완료 baseline
...
## 10. v0.1.0 (2026-07-20) — 백엔드/운영 성숙도 baseline
...
## 11. 회귀 baseline 종합 (TASK-088 → v0.6.0)
...
- `v0.6.1` — patch (회귀 baseline 변경 0 + 운영 가이드/CI 소폭)
- `v0.7.0` — minor (신규 기능 표면 큼: k8s adapter 확장(Helm·ArgoCD·per-build ns 정리) / 호스팅 status 캐시 / webhook 확장(Slack·재시도)). **HTTPS/TLS 는 도입하지 않음(범위 밖).**
