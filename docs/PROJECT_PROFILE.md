...
- Required-auth Compose overlay: `compose.dev.required-auth.yaml`은 `AUTH_MODE=required`, `AUTH_SECRET`, `RUNNER_AUTH_REQUIRED=true`, `RUNNER_AUTH_TOKEN`을 한 번에 정렬한다. 두 secret 값은 repository에 저장하지 않고 shell/secret manager에서 주입하며, 누락 시 `docker compose config` 단계에서 실패한다.
