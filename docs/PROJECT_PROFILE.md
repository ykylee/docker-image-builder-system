...
- OIDC client foundation: `apps/build-server/src/auth/oidc-client.ts`는 표준 discovery, authorization-code + PKCE(S256), issuer/audience/JWKS/nonce 검증, `sub`·role claim·`exp` principal 정규화를 제공한다. role claim은 `OIDC_ROLE_CLAIM`(기본 `roles`)으로 지정하며 `realm_access.roles` 같은 dotted path와 문자열/배열 값을 지원한다. client secret과 id token은 서버 경계를 벗어나지 않는다.
