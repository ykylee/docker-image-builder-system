...
- Phase 1 인증 foundation: `AUTH_SECRET`을 설정하면 관리자 API와 Runner 제어 API는 `Authorization: Bearer <HS256 token>`을 요구한다. 일반 build intake (`POST/GET /builds`, source 업로드, build detail/log/source 조회)는 불특정 사용자가 `appName`과 `requestedBy`를 payload로 제출할 수 있도록 공개한다. 토큰 payload는 `{ "sub": "<owner>", "roles": ["user"|"admin"], "exp": <unix seconds> }` 형식이며, 만료·위조 토큰은 보호 API에서 401, `admin` role 없는 `/admin/*` 요청은 403이다. `AUTH_SECRET` 미설정은 기존 self-dogfood 호환 모드이며 외부 노출 환경에서는 사용하지 않는다. 현재 토큰 발급은 외부 IdP/issuer 연동 전 단계로 운영 secret 보관 경로에서만 수행한다.
- 공개 build 데이터 정책(현재): build 목록·상세·로그·source 조회도 제출 흐름의 일부로 간주해 인증 없이 허용한다. 따라서 `requestedBy`는 신원 증명이 아니라 표시·필터용 등록 문자열이며, private build 데이터 보장이 필요한 환경은 별도 조회용 access token 정책을 도입해야 한다.
...
- `AUTH_MODE=legacy|required`를 지원한다. `legacy`(기본)는 기존 self-dogfood header 호환 모드이고, `required`는 `AUTH_SECRET`이 없으면 Build Server가 기동하지 않는다. Compose는 `AUTH_SECRET`을 환경변수로, Kubernetes control-plane 예시는 `dib-control-plane-auth` Secret의 `AUTH_SECRET` key로 주입한다(`optional: true`는 migration 기간의 호환 설정). 외부 운영에서는 Secret을 생성하고 `AUTH_MODE=required`로 승격해야 한다.
...
- required-mode 전환/rotation 절차: (1) Build Server와 토큰 issuer가 공유할 새 `AUTH_SECRET` 및 Runner별 서명 토큰을 준비하고, (2) Secret/env 저장소에 `AUTH_SECRET`, `AUTH_MODE=required`, `RUNNER_AUTH_TOKEN`을 함께 반영한 뒤, (3) Runner를 먼저 재기동해 새 토큰을 적용하고, (4) `POST /builds/claim` 왕복 및 public `POST /builds`를 smoke 검증한다. 검증 후 이전 secret·토큰을 폐기한다. 일반 사용자의 build 등록/조회 경로는 이 전환에서도 공개 정책을 유지한다.
