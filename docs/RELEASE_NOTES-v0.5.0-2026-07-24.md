...
`http://<context-path>.<HOSTING_BASE_HOST>/` 로 호스팅한다(평문 HTTP — HTTPS 미도입). 앱이 자기 subdomain 의 **루트**에서 서빙되므로 **절대경로 자산(`/main.js`)이 자연 동작 — 앱 무수정 지원**. path-prefix 가 요구하던 `APP_BASE_PATH` 규약·rewrite·stripPrefix 가 subdomain 에선 불필요하다.
...
- `HostedService.url` 이 `http://<cp>.<host>/` 로 조립된다(평문 — HTTPS/TLS 는 시스템 범위 밖, 필요 시 외부 LB 종단).
...
- **신규 env(선택)**: `RUNNER_HOSTING_BASE_HOST`(runner). **subdomain 사용 시 전제**: 클러스터 앞단에 **wildcard DNS**(`*.<host>`) — 운영. dev/e2e 는 **nip.io** magic DNS. **HTTPS/TLS 는 도입하지 않음**(평문 HTTP, 필요 시 외부 LB 종단).
