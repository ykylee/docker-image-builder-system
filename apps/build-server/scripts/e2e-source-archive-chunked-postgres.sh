#!/usr/bin/env bash
# TASK-106: e2e — Postgres backend source archive chunked upload (TASK-106).
#
...
step "[4/6] postgres bytea direct verify — each (build_id, idx) row"
for ((i = 0; i < NUM_CHUNKS; i++)); do
  part="$WORK_DIR/chunk.$(printf '%02d' "$i")"
  if [ ! -f "$part" ]; then
    part="${parts[$i]}"
  ...
  actual_sha="${actual%%|*}"
  actual_len="${actual##*|}"
  expected_len=$(wc -c < "$part" | tr -d ' ')
  if [ "$actual_sha" != "$expected_sha" ] || [ "$actual_len" != "$expected_len" ]; then
    err "chunk $i bytea mismatch (expected $expected_sha/$expected_len, got $actual_sha/$actual_len)"
  fi
done
ok "all $NUM_CHUNKS (build_id, idx) rows verified"
