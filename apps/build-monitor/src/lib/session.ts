 ...
 * userId / adminId 를 다시 읽었기 때문에, 같은 SPA 라우트 트리에서
 * Login 이나 AdminLogin 이 localStorage 에 쓴 값을 Header 가 알지 못했다
 * (storage 이벤트는 다른 탭에서만 fire). 결과적으로 로그인 직후에도
 * 상단 네비게이션 바의 계정/로그아웃이 즉시 노출되지 않았다.
 *
 * 이 store 는 writable<string | null> 두 개를 노출하고, set 시
 * localStorage 도 같이 쓴다. 다른 탭의 storage 이벤트는 여전히 read
 * 경로에서 자동 반영 (storage 이벤트는 setItem 호출과 무관하게 다른
 * document 의 변경에만 fire 되므로, 같은 탭에서는 store.set 이 곧
...
// 항상 Login/AdminLogin 의 submit 결과로만 바뀌어야 하고, 코드 어디서든
...
export const adminIdStore = createSessionStore(ADMIN_ID_KEY);
