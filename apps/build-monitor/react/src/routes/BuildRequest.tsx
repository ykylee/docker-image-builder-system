  ...
  return (
    <section className="page">
      ...
      <form className="form" onSubmit={submit} data-testid="req-form">
        {/* Section 1: App Identity & Source Archive */}
        <div className="form-section">
          <h2 className="section-title">1. App Identity & Source Archive</h2>
          <div className="grid">
            {FIELDS.filter((f) =>
              ["appName", "requestedBy", "objectKey", "checksumSha256", "sizeBytes", "entrypointPath"].includes(f.key)
            ).map((f) => {
        ...
        {/* Section 2: Build & Container Policy */}
        <div className="form-section">
          <h2 className="section-title">2. Build & Container Policy</h2>

          <div className="hosting-option" style={{ flexDirection: "column", alignItems: "stretch" }}>
            <span>
              <strong>Dockerfile Generation Mode</strong>
              <small>Choose whether a pre-existing Dockerfile is required or auto-generated by the runner.</small>
            </span>

            <div className="card-selector-grid" style={{ marginTop: "8px" }}>
              {[
                {
                  id: "required",
                  title: "Required",
                  badge: "Standard",
                  desc: "Requires a validated Dockerfile at repository root."
                },
                {
                  id: "auto",
                  title: "Auto-Generate",
                  badge: "Smart",
                  desc: "Runner auto-generates Dockerfile for static HTML & Node start scripts."
                }
              ].map((item) => (
                <div
                  key={item.id}
                  className={`select-card ${form.dockerfileMode === item.id ? "selected" : ""}`}
                  onClick={() =>
                    setForm((prev) => ({
                      ...prev,
                      dockerfileMode: item.id as FormState["dockerfileMode"]
                    }))
                  }
                >
                  <div className="select-card-head">
                    <span className="select-card-title">{item.title}</span>
                    <span className="select-card-badge">{item.badge}</span>
                  </div>
                  <span className="select-card-desc">{item.desc}</span>
                </div>
              ))}
            </div>

            {/* Hidden select for contract test compatibility */}
            <select
          ...
          <div className="grid" style={{ marginTop: "12px" }}>
            {FIELDS.filter((f) => ["dockerfilePath", "runtimePort"].includes(f.key)).map((f) => {
              const fieldError = submitFieldErrors.find((e) => e.path === f.errorPath);
              const status = fieldError
                ? ({ type: "error", message: fieldError.message } as const)
                : undefined;
        ...
        {/* Section 3: Hosting Tier & Resource Budget */}
        <div className="form-section">
          <h2 className="section-title">3. Hosting Tier & Resource Allocations</h2>

          <div className="hosting-option" style={{ flexDirection: "column", alignItems: "stretch" }}>
            <span>
              <strong>Target Hosting Tier</strong>
              <small>Resolved from size & resources. Auto keeps server default policy.</small>
            </span>

            <div className="card-selector-grid" style={{ marginTop: "8px" }}>
              {[
                { id: "", title: "Auto Policy", badge: "Server Default", desc: "Dynamically resolved based on resource limits." },
                { id: "sandbox", title: "Sandbox", badge: "100m / 128Mi", desc: "Lightweight dev workloads (1 Replica)." },
                { id: "standard", title: "Standard", badge: "250m / 512Mi", desc: "Production-ready single or dual replicas." },
                { id: "production", title: "Production", badge: "500m / 1Gi", desc: "High capacity multi-replica deployment." }
              ].map((tierItem) => (
                <div
                  key={tierItem.id || "auto"}
                  className={`select-card ${form.requestedTier === tierItem.id ? "selected" : ""}`}
                  onClick={() =>
                    setForm((prev) => ({
                      ...prev,
                      requestedTier: tierItem.id as FormState["requestedTier"]
                    }))
                  }
                >
                  <div className="select-card-head">
                    <span className="select-card-title">{tierItem.title}</span>
                    <span className="select-card-badge">{tierItem.badge}</span>
                  </div>
                  <span className="select-card-desc">{tierItem.desc}</span>
                </div>
              ))}
            </div>

            {/* Test Compatibility Select */}
            <select
          ...
          <div className="hosting-options" data-testid="req-hosting-options" style={{ marginTop: "12px" }}>
        ...
        {/* Section 4: Domain Routing & Ingress Options */}
        <div className="form-section">
          <h2 className="section-title">4. Domain Routing & Ingress Options</h2>
